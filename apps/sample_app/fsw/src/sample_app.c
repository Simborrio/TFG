...
CFE_Status_t SAMPLE_APP_Init(void)
{
    ...
    */
    ...
    else
    {
        ...
        /* --- NUEVO: inicializa la cabecera de nuestra telemetría de datos --- */
        CFE_MSG_Init(CFE_MSG_PTR(SAMPLE_APP_Data.DataTlm.TelemetryHeader),
                     CFE_SB_ValueToMsgId(SAMPLE_APP_DATA_TLM_MID),
                     sizeof(SAMPLE_APP_Data.DataTlm));

        /* --- NUEVO: inicializa la cabecera de la telemetría de sensores --- */
        CFE_MSG_Init(CFE_MSG_PTR(SAMPLE_APP_Data.SensorsTlm.TelemetryHeader),
                     CFE_SB_ValueToMsgId(SAMPLE_APP_SENSORS_TLM_MID),
                     sizeof(SAMPLE_APP_Data.SensorsTlm));
        /*
    ...
    if (status == CFE_SUCCESS)
    {
        ...
        */
        ...
        /* --- NUEVO: mutex para el cache de sensores + tarea hija MAVLink --- */
        status = OS_MutSemCreate(&SAMPLE_APP_Data.SensorCacheMutex, "SA_SensorMutex", 0);
        if (status != CFE_SUCCESS)
        {
            CFE_EVS_SendEvent(SAMPLE_APP_MAVLINK_ERR_EID,
                              CFE_EVS_EventType_ERROR,
                              "SAMPLE_APP: error creando mutex de sensores, RC = 0x%08lX",
                              (unsigned long)status);
        }
        else
        {
            status = CFE_ES_CreateChildTask(&SAMPLE_APP_Data.MavlinkTaskId,
                                            "SA_Mavlink",
                                            (CFE_ES_ChildTaskMainFuncPtr_t)&SAMPLE_APP_MavlinkTask,
                                            NULL,
            ...
            if (status != CFE_SUCCESS)
            {
                CFE_EVS_SendEvent(SAMPLE_APP_MAVLINK_ERR_EID,
                                  CFE_EVS_EventType_ERROR,
                                  "SAMPLE_APP: error creando tarea MAVLink, RC = 0x%08lX",
                                  (unsigned long)status);
            }
        }
        /* no queremos que un fallo de MAVLink tumbe la app entera */
        status = CFE_SUCCESS;
